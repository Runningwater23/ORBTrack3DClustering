	...
	int img_dbl, int descr_width, int descr_hist_bins)///
{
	...
	*feat = (struct feature*)calloc(n, sizeof(struct feature));//分配控件
	...
	int intvls, double sigma)
{
	IplImage*** gauss_pyr;
	//为了保证连续性，在每一层的顶层继续用高斯模糊生成3幅图像，所以高斯金字塔每组有intvls+3层，DOG金字塔每组有intvls+2层
	double* sig = (double*)calloc(intvls + 3, sizeof(double));//每层的sigma参数数组
	...
	double contr_thr, int curv_thr, CvMemStorage* storage, int windowHeight, int windowWidth)
{
	...
	for (o = 1; o < octvs; o++)//第o组
	for (i = 1; i <= intvls; i++)//遍i层
	for (winr = 0; winr < (dog_pyr[o][0]->height - 2 * SIFT_IMG_BORDER) / windowHeight; winr++)//滑动窗口所在的行
	for (winc = 0; winc < (dog_pyr[o][0]->width - 2 * SIFT_IMG_BORDER) / windowWidth; winc++)//滑动窗口所在的列
	{
		int leftTop = SIFT_IMG_BORDER + winr*windowHeight;//窗口内第一个点距图像顶部距离
		int topLeft = SIFT_IMG_BORDER + winc*windowWidth;//窗口内第一个点距图像左端距离
		LinkQueue maxQ;//构建一个空队列，用来存放最大值
		int initmax = InitQueue(&maxQ);
		LinkQueue minQ;//构建一个空队列，用来存放最小值
		...
		if (!QueueEmpty(maxQ))
		{
			QElemType max;
			DeQueue(&maxQ, &max);
			if (is_extremum(dog_pyr, o, i, max.r, max.c))//若是极值点
			{
				feat = interp_extremum(dog_pyr, o, i, r, c, intvls, contr_thr);
				if (feat)
				{
		...
		if (!QueueEmpty(minQ))
		{
			QElemType min;
			DeQueue(&minQ, &min);
			if (is_extremum(dog_pyr, o, i, min.r, min.c))//若是极值点
			{
				feat = interp_extremum(dog_pyr, o, i, r, c, intvls, contr_thr);
				if (feat)
				{
...
static struct feature* new_feature(void)
{
	...
	feat = (struct feature*)malloc(sizeof(struct feature));//分配空间
	...
	double scl, int d, int n)
{
	...
	//为直方图数组分配空间
	hist = (double***)calloc(d, sizeof(double**));//为第一维分配空间
	for (i = 0; i < d; i++)
	{
		hist[i] = (double**)calloc(d, sizeof(double*));//为第二维分配空间
		for (j = 0; j < d; j++)
			hist[i][j] = (double*)calloc(n, sizeof(double));//为第三维分配空间
	}
